...
class RegionalDatabaseManager:
    """Multi-database manager for regional banking architecture."""

    def __init__(self):
        self.central_database_path = Path(settings.database_dir) / settings.central_database
    ...
    def _initialize_databases(self):
        """Initialize all regional database connections."""
        # Central database
        central_url = f"sqlite:///{self.central_database_path}"
        self.engines['central'] = create_engine(
            central_url,
            connect_args={"check_same_thread": False},
            poolclass=StaticPool,
        )
        self.sessions['central'] = sessionmaker(
            autocommit=False, autoflush=False, bind=self.engines['central']
        )

        # Regional databases
        for region, config in settings.regional_databases.items():
            db_path = Path(settings.database_dir) / config['file']
            db_url = f"sqlite:///{db_path}"

            self.engines[region] = create_engine(
                db_url,
                connect_args={"check_same_thread": False},
                poolclass=StaticPool,
            )
            self.sessions[region] = sessionmaker(
                autocommit=False, autoflush=False, bind=self.engines[region]
            )
            self.regional_databases[region] = {
                'path': db_path,
                'config': config,
                'current_transactions': 0
            }

    def get_session(self, database: str = 'central'):
        """Get a database session for specified database."""
        if database not in self.sessions:
            raise ValueError(f"Database '{database}' not found")
        return self.sessions[database]()

    def get_file_size_mb(self, database: str) -> float:
        """Get current file size in MB."""
        if database == 'central':
            path = self.central_database_path
        else:
            path = self.regional_databases[database]['path']

        if path.exists():
            return path.stat().st_size / (1024 * 1024)
        return 0.0

    def can_add_transactions(self, region: str, count: int) -> bool:
        """Check if we can add more transactions without exceeding limits."""
        if region not in self.regional_databases:
            return False

        current = self.regional_databases[region]['current_transactions']
        max_allowed = self.regional_databases[region]['config']['max_transactions']

        return (current + count) <= max_allowed

    def update_transaction_count(self, region: str, count: int):
        """Update the transaction count for a region."""
        if region in self.regional_databases:
            self.regional_databases[region]['current_transactions'] += count

    def get_database_info(self) -> dict:
        """Get information about all databases."""
        info = {
            'central': {
                'path': str(self.central_database_path),
                'size_mb': self.get_file_size_mb('central'),
                'description': 'Central registry and reference data'
            }
        }

        for region in self.regional_databases:
            info[region] = {
                'path': str(self.regional_databases[region]['path']),
                'size_mb': self.get_file_size_mb(region),
                'max_transactions': self.regional_databases[region]['config']['max_transactions'],
                'current_transactions': self.regional_databases[region]['current_transactions'],
                'description': self.regional_databases[region]['config']['description']
            }
...
class DatabaseManager:
    ...
    def _get_fast_amount(self, transaction_type: str, transaction_subtype: str,
        ...
        if transaction_type == "WITHDRAWAL":
            ...
            if transaction_subtype == "MERCHANT_PAYMENT":
                amounts = amount_lookup["PAYMENT"].get(
                    profile['occupation'],
                    amount_lookup["PAYMENT"]["default"]
                )
                return -random.choice(amounts)
            else:
    ...
    def generate_regional_databases(self, force_regenerate: bool = True):
        """Generate GitHub-optimized regional banking databases with maximum realism."""

        logger.info("🏛️ Starting GitHub-optimized regional banking database generation...")
        logger.info("🎯 Focus: Maximum realism within GitHub file size limits")

        # Clear existing database files if force_regenerate is True
        if force_regenerate:
            logger.info("🧹 Clearing existing database files...")
            data_dir = Path(settings.database_dir)
            data_dir.mkdir(parents=True, exist_ok=True)

            # Remove central database
            central_path = data_dir / settings.central_database
            if central_path.exists():
                try:
                    central_path.unlink()
                    logger.info(f"  Removed {central_path}")
                except PermissionError:
                    logger.warning(f"  Could not remove {central_path} (file in use)")

            # Remove regional databases
            for region, config in settings.regional_databases.items():
                db_path = data_dir / config['file']
                if db_path.exists():
                    try:
                        db_path.unlink()
                        logger.info(f"  Removed {db_path}")
                    except PermissionError:
                        logger.warning(f"  Could not remove {db_path} (file in use)")

        # Initialize regional database manager
        regional_manager = RegionalDatabaseManager()

        try:
            # Phase 1: Generate Central Database (Reference Data)
            logger.info("📊 Phase 1: Generating central database with reference data...")
            self._generate_central_database(regional_manager, force_regenerate)

            # Phase 2: Generate Regional Databases with Economic Modeling
            logger.info("🗺️ Phase 2: Generating regional databases with economic modeling...")
            for region in settings.regional_databases.keys():
                self._generate_regional_database(region, regional_manager)

            # Phase 3: Generate Cross-Regional Patterns
            logger.info("🔄 Phase 3: Adding cross-regional transfer patterns...")
            self._add_cross_regional_patterns(regional_manager)

            # Phase 4: Validate File Sizes
            logger.info("📏 Phase 4: Validating GitHub file size compliance...")
            self._validate_file_sizes(regional_manager)

            logger.info("🎉 Regional database generation completed successfully!")
            logger.info("📋 Summary:")

            for db_name, info in regional_manager.get_database_info().items():
                logger.info(f"  {db_name}: {info['size_mb']:.1f}MB - {info['description']}")

        except Exception as e:
            logger.error(f"❌ Error in regional database generation: {e}")
            raise
    ...
    def generate_mock_data(self, force_regenerate: bool = True):
        ...
        try:
            ...
            # Phase 4: Fast balance calculation (5-10 seconds)
            logger.info("Phase 4: Calculating running balances in parallel...")
            ...
            for update_data in account_updates:
                session.query(Account).filter(Account.id == update_data['id']).update({
                    'balance': update_data['balance'],
                    'available_balance': update_data['available_balance']
                })
            session.commit()
...
class MultiDatabaseQueryEngine:
    """Intelligent query router for multi-database banking system."""

    def __init__(self):
        """Initialize multi-database query engine."""
        self.regional_manager = RegionalDatabaseManager()

        # Define which tables are in which databases
        self.table_locations = {
            # Central database tables (metadata)
            'clients': 'central',
            'accounts': 'central',
            'branches': 'central',
            'products': 'central',

            # Regional database tables (transactions)
            'transactions': 'regional'  # Special handling - can be in any regional DB
        }
    ...
    def execute_query(self, sql_query: str) -> List[dict]:
        """Execute query with intelligent multi-database routing."""
        try:
            ...
            routing_strategy = self._analyze_query(sql_query)

            # Execute based on strategy
            if routing_strategy['type'] == 'central_only':
                result = self._execute_central_query(sql_query)
            elif routing_strategy['type'] == 'specific_region':
                result = self._execute_regional_query(sql_query, routing_strategy['region'])
            elif routing_strategy['type'] == 'all_regions':
                result = self._execute_cross_regional_query(sql_query)
            elif routing_strategy['type'] == 'aggregated':
                result = self._execute_aggregated_query(sql_query, routing_strategy)
            else:
            ...
            logger.error(f"Multi-database query error: {e}")
            # Fallback to central database
            return self._execute_central_query(sql_query)

    def _analyze_query(self, sql_query: str) -> dict:
        """Analyze SQL query to determine optimal routing strategy."""
        query_upper = sql_query.upper()

        # Check for specific region in WHERE clause
        region_patterns = [
            r"WHERE.*REGION\s*=\s*['\"](\w+)['\"]",
            r"REGION\s*=\s*['\"](\w+)['\"]"
        ]
        ...
        for pattern in region_patterns:
            ...
            if match:
                ...
                if region in settings.regional_databases:
                    return {'type': 'specific_region', 'region': region}

        # Check if query involves transactions
        if 'TRANSACTIONS' in query_upper:
            # Check if it's an aggregation query (GROUP BY, COUNT, SUM, etc.)
            if any(keyword in query_upper for keyword in ['GROUP BY', 'COUNT', 'SUM', 'AVG', 'MAX', 'MIN']):
                return {'type': 'aggregated', 'tables': ['transactions']}
            else:
                return {'type': 'all_regions', 'tables': ['transactions']}

        # Check which tables are involved
        tables_in_query = []
        for table in self.table_locations.keys():
            if table.upper() in query_upper:
                tables_in_query.append(table)

        # If only central tables, route to central
        if all(self.table_locations.get(table) == 'central' for table in tables_in_query):
            return {'type': 'central_only', 'tables': tables_in_query}

        # Default to central for safety
        return {'type': 'central_only', 'tables': tables_in_query}

    def _execute_central_query(self, sql_query: str) -> List[dict]:
        """Execute query on central database only."""
        session = self.regional_manager.get_session('central')
        try:
            result = session.execute(text(sql_query))
            columns = result.keys()
            rows = result.fetchall()
    ...
    def _execute_regional_query(self, sql_query: str, region: str) -> List[dict]:
        """Execute query on specific regional database."""
        session = self.regional_manager.get_session(region)
        try:
            result = session.execute(text(sql_query))
            columns = result.keys()
            rows = result.fetchall()
    ...
    def _execute_cross_regional_query(self, sql_query: str) -> List[dict]:
        """Execute query across all regional databases and combine results."""
        all_results = []

        # Execute on each regional database
        for region in settings.regional_databases.keys():
            try:
                session = self.regional_manager.get_session(region)
                try:
                    ...
                    columns = result.keys()
                    rows = result.fetchall()
                    regional_results = [dict(zip(columns, row)) for row in rows]

                    # Add region information to each row
                    for row in regional_results:
                        row['_source_region'] = region.title()
    ...
    def _execute_aggregated_query(self, sql_query: str, routing_info: dict) -> List[dict]:
        """Execute aggregated query (GROUP BY, SUM, etc.) across regions."""
        # For aggregated queries, we need to modify the query to work per region
        # then aggregate the results

        # Simple approach: execute on each region and let the application handle aggregation
        # More complex aggregation logic could be added here for specific cases
        return self._execute_cross_regional_query(sql_query)


# Global database manager instance
db_manager = DatabaseManager()

# Enable multi-database mode if regional databases exist
try:
    from pathlib import Path
    data_dir = Path(settings.database_dir)
    central_db = data_dir / settings.central_database
    if central_db.exists() and any((data_dir / config['file']).exists() for config in settings.regional_databases.values()):
        db_manager.enable_multi_database_mode()
        logger.info("🚀 Banking system initialized with multi-database support")
except Exception as e:
    logger.info(f"Single database mode (multi-database not available): {e}")